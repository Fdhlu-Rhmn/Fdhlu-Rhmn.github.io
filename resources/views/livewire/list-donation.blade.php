    ...
    <div
        ...
        <div class="mb-4 flex justify-between">
            <div class="space-x-2">
                ...
                <button wire:click="setFilter('oldevents')"
                    class="px-4 py-2 {{ $filter == 'oldevents' ? 'bg-gray-400' : 'bg-gray-200' }} text-gray-700 rounded hover:bg-gray-300 focus:outline-none">Old
                    Events</button>
            </div>
            <div class="flex space-x-2">
                <input type="text" wire:model.defer="search" wire:keydown.enter="loadDonations"
                    placeholder="Search Donations..." class="px-4 py-2 border rounded focus:outline-none" />
                <button wire:click="loadDonations" class="px-4 py-2 bg-blue-500 text-white rounded">Search</button>
            </div>
        </div>
                ...
                <a href="donation/{{ $donation->id }}  " class="hover:bg-gray-300">
                    <div class="border-2 border-solid py-4 rounded-md shadow-lg -full bg-white font-roboto-serif">
                        <div class="mb-2 pr-2 bg-transparent text-right">
                            @php
                                $isNewDonation = $donation->tag == '1' && $donation->created_at->gt(\Carbon\Carbon::now()->subYear());
                            @endphp
                            @if ($isNewDonation)
                                <span class="px-2 py-1 bg-green-500 text-white text-xs rounded">New Events</span>
                            @elseif($donation->tag == '2')
                                <span class="px-2 py-1 bg-red-500 text-white text-xs rounded">Upcoming</span>
                            @else
                                <span class="px-2 py-1 bg-gray-500 text-white text-xs rounded">Old Event</span>
                            @endif
                        </div>                                                   
                        <div class="px-2">
                        ...
                        <div class="flex flex-col py-3"> 
                            ...
                            <span class="text-justify desc text-xs px-2 line-clamp-4 w-auto">
                                {{ $donation->desc }}
                            </span>
                            ...
                            <div class="flex justify-between">
                                <div class="flex flex-col">
                                    <div class="flex  space-x-2 justify-start pr-2 mt-4 px-2 items-center">
                                        ...
                                        <span class="font-semibold text-size">
                                            {{ $donation->date }}
                                        </span>
                                    </div>
                                    <div class="flex  space-x-2 justify-start pr-2 mt-4 px-2 items-center">
                                ...
                                <div class="flex items-end space-x-2 justify-end pr-2 mt-4">
                                    <span href="/donation-transfer" class="font-semibold text-xs warna-text">Donation</span>
